...
function ActionWorkflowCanvas({ workflow, jobs, now }) {
  ...
  const canvasFit = useMemo(() => {
    ...
    const scale = Math.min(1, Math.max(ACTION_FLOW_MIN_SCALE, (width - ACTION_FLOW_PADDING_X * 2) / baseLayoutSize.width));
    ...
    return {
      scale,
      left: Math.max(0, (width - scaledWidth) / 2),
  ...
  return (
    <div ref={viewportRef} className="relative overflow-hidden rounded-md border border-kumo-line bg-kumo-base" style={{ height: canvasFit.height }}>
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div
          ...
          style={{
            width: Math.max(layout.width * canvasFit.scale + canvasFit.left * 2, viewportSize.width || layout.width),
