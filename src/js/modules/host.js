...
export const hostMethods = {
  ...
  async runTerminalCommand() {
    ...
    try {
      const response = await fetch(`/api/server/task/command/${this.terminalServer.id}/sync`, {
        method: 'POST',
        headers: {
          ...this.getAuthHeaders(),
      ...
      if (data.success && data.data) {
        if (data.data.output) {
          this.terminalOutput += data.data.output;
          if (!data.data.output.endsWith('\n')) {
            this.terminalOutput += '\n';
          }
        }
        if (!data.data.successful && !data.data.output) {
          this.terminalOutput += `[命令执行失败]\n`;
        }
        // 显示执行时间
        if (data.data.delay > 0) {
          this.terminalOutput += `[耗时: ${data.data.delay}ms]\n`;
        }
      } else {
        this.terminalOutput += `[错误] ${data.error || '执行失败'}\n`;
      }
    } catch (error) {
