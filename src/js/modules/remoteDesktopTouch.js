...
export function accelerateTrackpadDelta(deltaX, deltaY, elapsedMs = 16) {
  ...
  const gain = speed <= 0.25
    ? 1
    : 1 + clamp((speed - 0.25) / 1.25, 0, 1) * 1.6;
...
export function remoteCursorPoint(position, viewport, video, fillMode, transform = { scale: 1, x: 0, y: 0 }) {
  ...
  return {
    x: Number(transform.x || 0) + (offsetX + clamp(Number(position?.x || 0), 0, 1) * renderedWidth) * Number(transform.scale || 1),
    y: Number(transform.y || 0) + (offsetY + clamp(Number(position?.y || 0), 0, 1) * renderedHeight) * Number(transform.scale || 1),
  };
}
