...
- 使用 Kumo `Table`。
- 业务表格通过 `AppTable columns` 声明 `primary`、`status`、`datetime`、`actions-*` 等语义列角色；不要新增纯百分比列宽。
- 工具列固定、内容列弹性增长。宽表应让两个或以上有阅读价值的内容列共同分配余量；固定布局必须有语义列规格或有效 `colgroup`，不得让所有列无差别平均增长。
- 数据密集表格默认不换行：`whitespace-nowrap`。
- 内容过长使用 `truncate`、`title`、`ClipboardText` 或详情弹窗，不让单元格撑爆布局。
- 表体默认垂直居中；操作、选择、状态和控制列必须垂直居中，多行内容列可以显式顶部对齐。
- 移动端保持列的最小可读宽度，并在表格框架内部横向滚动，不把横向溢出传播到页面。
- 需要用户调节列宽时使用 `Table.ResizeHandle` 和 `useTableResize.js`，并遵守角色宽度边界。
