...
**当前阶段**: Go Manifest 后端是默认运行架构

- **当前状态**: 路由清单由 `backend-go/internal/manifest/manifest.go` 管理，当前 route inventory 为 179 条 Go-owned 路由。
- **维护重点**: 保持前端路由、Go manifest 和后端 dispatch 三侧一致。
- **历史背景**: 旧 Node.js / Express sidecar 资料仅作为迁移历史，不作为新增模块的默认模型。
...
3. **Manifest 治理**: Go route manifest 是路由 ownership、auth 和响应模式的源头
...
- [前端最佳实践](./前端开发最佳实践.md)
- [Go 后端指南](./GO后端启动指南.md)
- [Go 后端迁移状态](./backend-migration-complete.md)
